...
export default function ProfilePage() {
  ...
  if (appContext.status !== 'authenticated') {
    // Guests might have a limited profile view or none at all
    // For now, redirect or show a message if not a fully authenticated user
            ...
            <div className="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Your current stay information.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <p><strong>Reservation Number:</strong> {appContext.guestData.reservation.reservationNumber}</p>
                        <p><strong>Branch:</strong> {appContext.guestData.reservation.branchName}</p>
                        <p><strong>Stay Period:</strong> {new Date(appContext.guestData.reservation.startDate).toLocaleDateString()} - {new Date(appContext.guestData.reservation.endDate).toLocaleDateString()}</p>
                        <UserTypeBadge type={appContext.guestData.reservation.type} className="mt-2" />
                         <Button onClick={logout} variant="outline" className="w-full mt-6">End Guest Session</Button>
                    </CardContent>
                </Card>
            </div>
        );
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3 p-3 border rounded-md">
                ...
                <div>
                  <Label htmlFor="profileName" className="text-xs text-muted-foreground">Full Name</Label>
              ...
              <div className="flex items-center space-x-3 p-3 border rounded-md">
                ...
                <div>
                  <Label htmlFor="profileEmail" className="text-xs text-muted-foreground">Email Address</Label>
              ...
              <div className="flex items-center space-x-3 p-3 border rounded-md">
                ...
                <div>
                  <Label htmlFor="profilePhone" className="text-xs text-muted-foreground">Phone Number</Label>
          ...
          <div>
            ...
            {user.activeReservations.length > 0 ? (
              ...
              <p className="text-muted-foreground">No active services linked to your account.</p>
            )}
             <Button variant="outline" className="mt-4 w-full sm:w-auto">
                Add New Service / Reservation
            </Button>
          </div>
          ...
          <div>
            ...
            {/* Placeholder for 2FA */}
            <p className="text-xs text-muted-foreground mt-2">Consider enabling Two-Factor Authentication for enhanced security.</p>
          </div>
