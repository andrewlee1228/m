...
interface Facility {
  ...
  imageHint: string; // For picsum placeholder data-ai-hint
  availableSlots?: string[]; // e.g. ["09:00 - 10:00", "10:00 - 11:00"]
}
...
export default function FacilitiesPage() {
     ...
     return (
       <div className="flex items-center justify-center h-full">
        <Card className="max-w-md text-center p-8">
          ...
          <CardDescription className="mt-2">Facility booking is available for LongStay residents.</CardDescription>
        </Card>
      </div>
    );
  }
  ...
  const handleBookFacility = () => {
    if (selectedFacility && selectedDate && (selectedFacility.availableSlots ? selectedSlot : true)) {
      alert(`Booking ${selectedFacility.name} for ${format(selectedDate, "PPP")} ${selectedSlot ? `at ${selectedSlot}`: ''}. (Mocked)`);
      // Reset selection
      setSelectedFacility(null);
      setSelectedSlot(null);
    } else {
      alert("Please select a facility, date, and time slot (if applicable).");
    }
  };
  ...
  return (
    <div className="space-y-8">
      <CardHeader className="px-0 pt-0">
        ...
        <CardDescription>
            Reserve amenities at {activeReservation.branchName}. Select a facility, date, and time slot.
        </CardDescription>
      </CardHeader>
      ...
      {selectedFacility && (
        <Card className="mt-8 shadow-lg">
          ...
          <CardContent className="space-y-6">
            <div>
                <Label className="font-medium block mb-1.5">Select Date</Label>
                <Popover>
            ...
            {selectedFacility.availableSlots && selectedFacility.availableSlots.length > 0 && (
              <div>
                <Label className="font-medium block mb-1.5">Select Time Slot</Label>
          ...
          <CardFooter>
            ...
            >
              Confirm Booking for {selectedFacility.name}
            </Button>
          </CardFooter>
        </Card>
      )}
