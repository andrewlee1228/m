...
import { Eye, EyeOff, LogIn, Ticket, Phone } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleStayGuestLogin = async (e: FormEvent) => {
    ...
    if (loginAsStayGuest(reservationNumber, phoneNumber)) {
            ...
            <TabsContent value="account">
              <form onSubmit={handleAccountLogin} className="space-y-6 mt-4">
                ...
                <Button type="submit" className="w-full bg-primary hover:bg-primary/90" disabled={isLoading}>
                  {isLoading ? 'Logging in...' : <> <LogIn className="mr-2 h-4 w-4" /> Login with Account </>}
                </Button>
              ...
              <form onSubmit={handleStayGuestLogin} className="space-y-6 mt-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                   ...
                   <p className="text-xs text-muted-foreground pt-1">Used to verify your booking. <Link href="/find-reservation" legacyBehavior><a className="text-primary hover:underline">Help finding reservation?</a></Link></p>
                </div>
                <Button type="submit" className="w-full bg-accent hover:bg-accent/90" disabled={isLoading}>
                 {isLoading ? 'Verifying...' : <> <LogIn className="mr-2 h-4 w-4" />  Access Your Stay </>}
                </Button>
              </form>
            </TabsContent>
          </Tabs>
